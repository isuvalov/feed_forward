...
%     if (mod(zd,InterpolateRate)==zPOS)
%         global_ce=1;
%     else
%         global_ce=0;
%     end;

    global_ce_a=[global_ce_a global_ce];
...
%         val=rx_transfer_filt(zd+2);
%         fi_error=val .* conj(modulate(mod_engine,demodulate(demod_engine,val)));
%         mu=-angle(fi_error);   


    farrow_a=[rx_transfer_filt(zd+2+zPOS); farrow_a(1:3)];
